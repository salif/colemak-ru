...
翻译： [english](LINUX.md), [русский](LINUX.ru.md), [español](LINUX.es.md), [العربية](LINUX.ar.md), [português](LINUX.pt.md), [bahasa](LINUX.id.md), [türkçe](LINUX.tr.md), [български](LINUX.bg.md), [esperanto](LINUX.eo.md)
...
**2.** 打开文件 `/usr/share/X11/xkb/symbols/ru` 并将以下文本块附加到文件末尾:
...
**3.** 打开文件 `/usr/share/X11/xkb/rules/evdev.xml` 并在变体后面插入以下文本块 `Russian (typewriter)`:
...
**4.** 然后加 `Russian (Colemak)` 通过桌面环境的设置.
